...
describe('verifyContract', () => {
  ...
  test('verify contract (blockscout)', async () => {
    const instanceBlockscout = new Etherscan(
        "BLOCKSCOUT",
        "https://eth-holesky.blockscout.com/api",
    ...
    await verifyContract(await createPublicClient({
      chain: holesky,
      transport: http('https://holesky.drpc.org')
    }), instanceBlockscout, artifacts, { address: "0x67e2eb9e8c38b2597e835a3914822f92bb43a193" })
    expect(isVerifiedSpy).toHaveBeenCalled()
    expect(verifySpy).toHaveBeenCalled()
    expect(getVerificationStatusSpy).toHaveBeenCalled()
  })

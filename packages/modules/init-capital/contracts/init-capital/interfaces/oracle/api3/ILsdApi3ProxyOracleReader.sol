...
interface ILsdApi3ProxyOracleReader is IBaseOracle {
    struct DataFeedInfo {
        address dataFeedProxy; // dataFeedProxy for the lsd exchange rate (dApi)
    ...
    event SetDataFeed(address indexed token, address dataFeedProxy);
    ...
    function dataFeedInfos(address _token)
        external
        view
        returns (address dataFeedProxy, address quoteToken, uint96 maxStaleTime);

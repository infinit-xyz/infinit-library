...
import { SetMaxStaleTimesTxBuilder } from '@actions/subactions/tx-builders/LsdApi3ProxyOracleReader/setMaxStaleTimes'

import { TestChain, TestInfinitWallet } from '@infinit-xyz/test'

class TestSetApi3ProxyOracleReaderMaxStaleTimesSubAction extends SetLsdApi3ProxyOracleReaderMaxStaleTimesSubAction {
  public override setTxBuilders(): void {
    super.setTxBuilders()
  }
}

describe('SetLsdApi3ProxyOracleReaderMaxStaleTimesSubAction', async () => {
  const tester = ARBITRUM_TEST_ADDRESSES.tester
  const client = new TestInfinitWallet(TestChain.arbitrum, tester)
  const oneDay = 60n * 60n * 24n
  test('test calldata should be matched', async () => {
    const params: SetLsdApi3ProxyOracleReaderMaxStaleTimesSubActionParams = {

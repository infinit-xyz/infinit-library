...
import { AddAssetListingAdminAction } from '@actions/addAssetListingAdmin'
import { AddAssetListingAdminSubAction } from '@actions/subactions/addAssetListingAdmin'

import { TestChain, TestInfinitWallet } from '@infinit-xyz/test'

vi.mock('@actions/subactions/addAssetListingAdmin')

describe('AddAssetListingAdminAction', () => {
  let action: AddAssetListingAdminAction
  ...
  describe('run', () => {
    action = new AddAssetListingAdminAction({
      params: {
    ...
    test('should run sub-actions successfully', async () => {
      const mockSubActionExecute = vi.mocked(AddAssetListingAdminSubAction.prototype.execute)
    ...
    test('should throw an error if sub-action validation fails', async () => {
      const mockSubActionValidate = vi.mocked(AddAssetListingAdminSubAction.prototype.validate)
    ...
    test('should handle cache correctly', async () => {
      const cache: InfinitCache = {
        name: 'AddAssetListingAdminAction',
        subActions: [
          {
            name: 'MockSubAction',
            transactions: [
    ...
    test('should handle callback correctly', async () => {
      ...
      const mockSubAction = vi.mocked(AddAssetListingAdminSubAction.prototype)

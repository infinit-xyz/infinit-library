...
import { RemoveAssetListingAdminTxBuilder } from '@actions/subactions/tx-builders/aclManager/removeAssetListingAdmin'
...
describe('RemoveAssetListingAdminTxBuilder', () => {
  let txBuilder: RemoveAssetListingAdminTxBuilder
  ...
  test('validate client not default admin', async () => {
    txBuilder = new RemoveAssetListingAdminTxBuilder(badClient, {
  ...
  test('validate add asset listing admin', async () => {
    txBuilder = new RemoveAssetListingAdminTxBuilder(client, {
      aclManager: aclManager,
      assetListingAdmin: bob,
    })
  ...
  async function checkTx(tx: TransactionData) {
    ...
    expect(decodedData.functionName).toBe('removeAssetListingAdmin')

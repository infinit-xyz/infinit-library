...
import { RemoveEmergencyAdminTxBuilder } from '@actions/subactions/tx-builders/aclManager/removeEmergencyAdmin'
...
describe('RemoveEmergencyAdminTxBuilder', () => {
  let txBuilder: RemoveEmergencyAdminTxBuilder
  ...
  test('validate client not default admin', async () => {
    txBuilder = new RemoveEmergencyAdminTxBuilder(badClient, {
  ...
  test('validate remove emergency admin', async () => {
    txBuilder = new RemoveEmergencyAdminTxBuilder(client, {
      aclManager: aclManager,
      emergencyAdmin: bob,
    })
  ...
  async function checkTx(tx: TransactionData) {
    ...
    expect(decodedData.functionName).toBe('removeEmergencyAdmin')

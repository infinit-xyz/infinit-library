...
import { AddRiskAdminACLManagerTxBuilder } from '@actions/subactions/tx-builders/aclManager/addRiskAdmin'
...
describe('AddRiskAdminACLManagerTxBuilder', () => {
  let txBuilder: AddRiskAdminACLManagerTxBuilder
  ...
  test('validate client not default admin', async () => {
    txBuilder = new AddRiskAdminACLManagerTxBuilder(badClient, {
  ...
  test('validate add risk admin', async () => {
    txBuilder = new AddRiskAdminACLManagerTxBuilder(client, {
      aclManager: aclManager,
      riskAdmin: bob,
    })
  ...
  async function checkTx(tx: TransactionData) {
    ...
    expect(decodedData.functionName).toBe('addRiskAdmin')

...
import { RemovePoolAdminTxBuilder } from '@actions/subactions/tx-builders/aclManager/removePoolAdmin'
...
describe('RemovePoolAdminTxBuilder', () => {
  let txBuilder: RemovePoolAdminTxBuilder
  ...
  test('validate client not default admin', async () => {
    txBuilder = new RemovePoolAdminTxBuilder(badClient, {
  ...
  test('validate remove pool admin', async () => {
    txBuilder = new RemovePoolAdminTxBuilder(client, {
      aclManager: aclManager,
      poolAdmin: bob,
    })
  ...
  async function checkTx(tx: TransactionData) {
    ...
    expect(decodedData.functionName).toBe('removePoolAdmin')

...
export type RemovePoolAdminParams = {
  aclManager: Address
  poolAdmin: Address
}

export class RemovePoolAdminTxBuilder extends TxBuilder {
  private aclManager: Address
  private poolAdmin: Address

  constructor(client: InfinitWallet, params: RemovePoolAdminParams) {
    super(RemovePoolAdminTxBuilder.name, client)
  ...
  async buildTx(): Promise<TransactionData> {
    ...
    const callData = encodeFunctionData({ abi: aclManagerArtifact.abi, functionName: 'removePoolAdmin', args: [this.poolAdmin] })
  ...
  public async validate(): Promise<void> {
    // check zeroAddress
    if (this.poolAdmin === zeroAddress) throw new ValidateInputValueError('POOL_ADMIN_CANNOT_BE_ZERO_ADDRESS')
